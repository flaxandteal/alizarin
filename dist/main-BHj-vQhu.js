...
let __tla = (async () => {
  ...
  utils = Object.freeze(Object.defineProperty({
    ...
    generateUuidv5,
    getCurrentLanguage: getCurrentLanguage$2,
    serializeValuesMap,
    setCurrentLanguage: setCurrentLanguage$2,
    slugify: slugify$1
  }, Symbol.toStringTag, {
    value: "Module"
  }));
  let _backend = "wasm";
  let _napiModule = null;
  let _wasmModule = null;
  setWasmModule = function(mod) {
    _wasmModule = mod;
  };
  ...
  setNapiModule = function(mod) {
    _napiModule = mod;
    _backend = "napi";
    globalThis.__alizarin_napi = mod;
  };
  function safeStringify(value) {
    if (value === null || value === void 0) return JSON.stringify(value);
    if (typeof value === "string") return value;
    const isNapiObject = value.constructor && value.constructor.name !== "Object" && value.constructor.name !== "Array" && JSON.stringify(value) === "{}";
    if (isNapiObject) {
      throw new Error(`Cannot JSON.stringify a ${value.constructor.name} instance \u2014 NAPI class getters are not enumerable. Use a direct method instead.`);
    }
    return JSON.stringify(value);
  }
  function createInstanceWrapperForResource(resource, registry) {
    if (_backend === "napi") {
    ...
    const { newWASMResourceInstanceWrapperForResource: newWASMResourceInstanceWrapperForResource2 } = getWasmModule();
    return newWASMResourceInstanceWrapperForResource2(resource);
  }
  function loadTilesFromResource(wrapper, resource, assumeComprehensive) {
    var _a2;
    if (_backend === "napi") {
      wrapper.loadTilesFromResource(JSON.stringify(resource));
    } else if (((_a2 = resource.constructor) == null ? void 0 : _a2.name) === "StaticResource") {
      wrapper.loadTilesFromResource(resource, assumeComprehensive);
    } else {
      wrapper.loadTiles(resource.tiles || []);
    }
  }
  function createInstanceWrapperForModel(graphId, resourceId) {
    if (_backend === "napi") {
      const napi = getNapiModule();
      if (!napi) throw new Error("NAPI backend selected but @alizarin/napi not available");
  ...
  createResourceRegistry = function() {
    ...
    const { StaticResourceRegistry: StaticResourceRegistry2 } = getWasmModule();
    return new StaticResourceRegistry2();
  };
  let _rdmCache = null;
  function getRdmCache() {
    ...
    if (_backend === "napi") {
      ...
      const { WasmRdmCache: WasmRdmCache2 } = getWasmModule();
      _rdmCache = new WasmRdmCache2();
    }
    return _rdmCache;
  }
  registerExtensionHandler$1 = function(datatype, handler) {
    if (_backend === "napi") {
      const napi = getNapiModule();
  ...
  function createResourceModelWrapper(wkrm, graph, defaultAllow) {
    if (_backend === "napi") {
      const napi = getNapiModule();
      if (!napi) throw new Error("NAPI backend selected but @alizarin/napi not available");
    ...
    const { WASMResourceModelWrapper: WASMResourceModelWrapper2 } = getWasmModule();
    return new WASMResourceModelWrapper2(wkrm, graph, defaultAllow);
  }
  function toPascalCase(s) {
    return s.replace(/[_-]/g, " ").split(/\s+/).filter((word) => word.length > 0).map((word) => word.charAt(0).toUpperCase() + word.slice(1)).join("");
  }
  function extractTranslatableName(name) {
  ...
  createWKRM = function(meta) {
    if (_backend === "napi") {
      return new TSWkrm(meta);
    }
    const { WKRM: WKRM2 } = getWasmModule();
    return new WKRM2(meta);
  };
  getWKRMClass = function() {
    if (_backend === "napi") return TSWkrm;
    const { WKRM: WKRM2 } = getWasmModule();
    return WKRM2;
  };
  function createWasmType(className, data) {
    if (_backend === "napi") {
      return data;
    }
  ...
  function createStaticGraphMeta(data) {
    return createWasmType("StaticGraphMeta", data);
  }
  function createStaticGraph$1(data) {
    if (_backend === "napi") {
      const graph = {
        ...data
      };
      ...
      graph.nodegroups = graph.nodegroups ? [
        ...graph.nodegroups
      ] : [];
      ...
      graph.pushNode = function(node) {
        this.nodes.push(node);
      };
      graph.pushEdge = function(edge) {
        this.edges.push(edge);
      };
      graph.pushNodegroup = function(ng) {
        this.nodegroups.push(ng);
      };
      graph.pushCard = function(card) {
        this.cards.push(card);
      };
      graph.pushCardXNodeXWidget = function(cxnxw) {
        this.cards_x_nodes_x_widgets.push(cxnxw);
      };
      graph.copy = function() {
        return createStaticGraph$1(this);
      };
  ...
  function createStaticNode(data) {
    return createWasmType("StaticNode", data);
  }
  function createStaticTile(data) {
    if (_backend === "napi") {
      const napi = getNapiModule();
      const tile = new napi.NapiStaticTile(data.nodegroup_id || "", data.tileid || null, data.sortorder ?? null, data.resourceinstance_id || "", data.parenttile_id || null);
      if (data.data) {
        const entries = data.data instanceof Map ? data.data : Object.entries(data.data || {});
  ...
  function createStaticTranslatableString(data) {
    ...
    const { StaticTranslatableString: StaticTranslatableString2 } = getWasmModule();
    return new StaticTranslatableString2(data);
  }
  parseStaticGraph = function(jsonText) {
    var _a2;
    if (_backend === "napi") {
      const napi = getNapiModule();
      if ((_a2 = napi == null ? void 0 : napi.NapiStaticGraph) == null ? void 0 : _a2.fromJsonString) {
    ...
    const { StaticGraph: StaticGraph2 } = getWasmModule();
    return StaticGraph2.fromJsonString(jsonText);
  };
  function parseStaticResources(jsonText) {
    var _a2;
    if (_backend === "napi") {
      const parsed = JSON.parse(jsonText);
      const resources = (_a2 = parsed == null ? void 0 : parsed.business_data) == null ? void 0 : _a2.resources;
      return Array.isArray(resources) ? resources : [];
    }
    const { StaticResource: StaticResource2 } = getWasmModule();
    return StaticResource2.fromBusinessDataJsonString(jsonText);
  }
  function buildAliasToCollectionMap$1(graphJson, resolvableDatatypes, configKeys) {
    if (_backend === "napi") {
      const napi = getNapiModule();
      if (!napi) throw new Error("NAPI backend selected but @alizarin/napi not available");
  ...
  function findNeededCollections$1(treeJson, aliasToCollection) {
    if (_backend === "napi") {
      const napi = getNapiModule();
      if (!napi) throw new Error("NAPI backend selected but @alizarin/napi not available");
  ...
  function getDefaultResolvableDatatypes$1() {
    if (_backend === "napi") {
      const napi = getNapiModule();
      if (!napi) throw new Error("NAPI backend selected but @alizarin/napi not available");
  ...
  function getDefaultConfigKeys$1() {
    if (_backend === "napi") {
      const napi = getNapiModule();
      if (!napi) throw new Error("NAPI backend selected but @alizarin/napi not available");
  ...
  getMemoryUsage = function(registry) {
    let jsHeapUsed = 0;
    let jsHeapTotal = 0;
    let rss = 0;
    let wasmMemory = 0;
  ...
  formatMemoryUsage = function(memOrRegistry) {
    let mem;
    if (!memOrRegistry) {
      mem = getMemoryUsage();
  ...
  autoDetectBackend = function() {
    var _a2, _b2;
    if (typeof process !== "undefined" && ((_a2 = process.env) == null ? void 0 : _a2.ALIZARIN_BACKEND)) {
      const env = process.env.ALIZARIN_BACKEND.toLowerCase();
      if (env === "napi" || env === "wasm") return env;
    }
    if (typeof process !== "undefined" && ((_b2 = process.versions) == null ? void 0 : _b2.node)) {
      const napi = getNapiModule();
      if (napi) return "napi";
    }
    return "wasm";
...
${val.stack}`;
  ...
  function newWASMResourceInstanceWrapperForModel(graph_id, resource_id) {
    ...
    var ptr1 = isLikeNone(resource_id) ? 0 : passStringToWasm0(resource_id, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    var len1 = WASM_VECTOR_LEN;
    const ret = wasm.newWASMResourceInstanceWrapperForModel(ptr0, len0, ptr1, len1);
    if (ret[2]) {
  ...
  function globalRdmLookupLabel(collection_id, concept_id, language) {
    ...
    const len2 = WASM_VECTOR_LEN;
    const ret = wasm.globalRdmLookupLabel(ptr0, len0, ptr1, len1, ptr2, len2);
    let v42;
    if (ret[0] !== 0) {
  ...
  function getCurrentLanguage$1() {
    ...
    let deferred1_1;
    try {
      const ret = wasm.getCurrentLanguage();
      deferred1_0 = ret[0];
      deferred1_1 = ret[1];
      return getStringFromWasm0(ret[0], ret[1]);
    } finally {
  ...
  function batchTreesToTiles(trees_json, graph, from_camel, strict, id_keys_json, random_ids) {
    const ptr0 = passStringToWasm0(trees_json, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len0 = WASM_VECTOR_LEN;
    _assertClass(graph, StaticGraph$1);
    var ptr1 = isLikeNone(id_keys_json) ? 0 : passStringToWasm0(id_keys_json, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    var len1 = WASM_VECTOR_LEN;
    const ret = wasm.batchTreesToTiles(ptr0, len0, graph.__wbg_ptr, from_camel, strict, ptr1, len1, isLikeNone(random_ids) ? 16777215 : random_ids ? 1 : 0);
    if (ret[2]) {
      throw takeFromExternrefTable0(ret[1]);
    }
    return takeFromExternrefTable0(ret[0]);
  ...
  function labelsToDeterministicString(labels) {
    ...
    let deferred2_1;
    try {
      const ret = wasm.labelsToDeterministicString(labels);
      var ptr1 = ret[0];
      var len1 = ret[1];
      if (ret[3]) {
        ptr1 = 0;
  ...
  } : new FinalizationRegistry((ptr) => wasm.__wbg_exampleedgewrapper_free(ptr >>> 0, 1));
  class ExampleEdgeWrapper {
    static __wrap(ptr) {
      ptr = ptr >>> 0;
      const obj = Object.create(ExampleEdgeWrapper.prototype);
      obj.__wbg_ptr = ptr;
      ExampleEdgeWrapperFinalization.register(obj, obj.__wbg_ptr, obj);
      return obj;
    }
    __destroy_into_raw() {
      const ptr = this.__wbg_ptr;
      this.__wbg_ptr = 0;
      ExampleEdgeWrapperFinalization.unregister(this);
      return ptr;
    }
    free() {
      const ptr = this.__destroy_into_raw();
      wasm.__wbg_exampleedgewrapper_free(ptr, 0);
    }
    get edgeid() {
      const ret = wasm.exampleedgewrapper_get_edgeid(this.__wbg_ptr);
      return ret;
    }
    get rangenode_id() {
      const ret = wasm.exampleedgewrapper_get_rangenode_id(this.__wbg_ptr);
      return ret;
    }
    get domainnode_id() {
      const ret = wasm.exampleedgewrapper_get_domainnode_id(this.__wbg_ptr);
      return ret;
    }
    get ontologyproperty() {
      const ret = wasm.exampleedgewrapper_get_ontologyproperty(this.__wbg_ptr);
      return ret;
    }
    constructor(json_data) {
      const ret = wasm.exampleedgewrapper_new(json_data);
      if (ret[2]) {
        throw takeFromExternrefTable0(ret[1]);
      }
      this.__wbg_ptr = ret[0] >>> 0;
      ExampleEdgeWrapperFinalization.register(this, this.__wbg_ptr, this);
      return this;
    }
    copy() {
      const ret = wasm.exampleedgewrapper_copy(this.__wbg_ptr);
      return ExampleEdgeWrapper.__wrap(ret);
    }
    toJSON() {
      const ret = wasm.exampleedgewrapper_toJSON(this.__wbg_ptr);
      return ret;
    }
    get name() {
      const ret = wasm.exampleedgewrapper_get_name(this.__wbg_ptr);
      return ret;
    }
  }
  if (Symbol.dispose) ExampleEdgeWrapper.prototype[Symbol.dispose] = ExampleEdgeWrapper.prototype.free;
  ...
  let PseudoValue$1 = class PseudoValue2 {
    ...
    get node() {
      ...
      return StaticNode$1.__wrap(ret);
    }
    get size() {
  ...
  let StaticCard$1 = class StaticCard2 {
    static __wrap(ptr) {
      ptr = ptr >>> 0;
      const obj = Object.create(StaticCard2.prototype);
      obj.__wbg_ptr = ptr;
  ...
  if (Symbol.dispose) StaticCard$1.prototype[Symbol.dispose] = StaticCard$1.prototype.free;
  ...
  let StaticCardsXNodesXWidgets$1 = class StaticCardsXNodesXWidgets2 {
    static __wrap(ptr) {
      ptr = ptr >>> 0;
      const obj = Object.create(StaticCardsXNodesXWidgets2.prototype);
      obj.__wbg_ptr = ptr;
  ...
  if (Symbol.dispose) StaticCardsXNodesXWidgets$1.prototype[Symbol.dispose] = StaticCardsXNodesXWidgets$1.prototype.free;
  ...
  let StaticEdge$1 = class StaticEdge2 {
    static __wrap(ptr) {
      ptr = ptr >>> 0;
      const obj = Object.create(StaticEdge2.prototype);
      obj.__wbg_ptr = ptr;
  ...
  if (Symbol.dispose) StaticEdge$1.prototype[Symbol.dispose] = StaticEdge$1.prototype.free;
  ...
  let StaticGraph$1 = class StaticGraph2 {
    static __wrap(ptr) {
      ptr = ptr >>> 0;
      const obj = Object.create(StaticGraph2.prototype);
      obj.__wbg_ptr = ptr;
    ...
    pushNodegroup(nodegroup) {
      _assertClass(nodegroup, StaticNodegroup$1);
    ...
    static fromJsonString(json_str) {
      ...
      return StaticGraph2.__wrap(ret[0]);
    }
    get jsonldcontext() {
    ...
    pushCardXNodeXWidget(cxnxw) {
      _assertClass(cxnxw, StaticCardsXNodesXWidgets$1);
    ...
    copy() {
      ...
      return StaticGraph2.__wrap(ret);
    }
    toJSON() {
  ...
  if (Symbol.dispose) StaticGraph$1.prototype[Symbol.dispose] = StaticGraph$1.prototype.free;
  ...
  let StaticGraphMeta$1 = class StaticGraphMeta2 {
    static __wrap(ptr) {
      ptr = ptr >>> 0;
      const obj = Object.create(StaticGraphMeta2.prototype);
      obj.__wbg_ptr = ptr;
    ...
    getRoot() {
      ...
      return ret === 0 ? void 0 : StaticNode$1.__wrap(ret);
    }
    get slug() {
    ...
    setRoot(value) {
      let ptr0 = 0;
      if (!isLikeNone(value)) {
        _assertClass(value, StaticNode$1);
        ptr0 = value.__destroy_into_raw();
      }
  ...
  if (Symbol.dispose) StaticGraphMeta$1.prototype[Symbol.dispose] = StaticGraphMeta$1.prototype.free;
  ...
  let StaticNode$1 = class StaticNode2 {
    static __wrap(ptr) {
      ptr = ptr >>> 0;
      const obj = Object.create(StaticNode2.prototype);
      obj.__wbg_ptr = ptr;
  ...
  if (Symbol.dispose) StaticNode$1.prototype[Symbol.dispose] = StaticNode$1.prototype.free;
  ...
  let StaticNodegroup$1 = class StaticNodegroup2 {
    static __wrap(ptr) {
      ptr = ptr >>> 0;
      const obj = Object.create(StaticNodegroup2.prototype);
      obj.__wbg_ptr = ptr;
    ...
    copy() {
      ...
      return StaticNodegroup2.__wrap(ret);
    }
    toJSON() {
  ...
  if (Symbol.dispose) StaticNodegroup$1.prototype[Symbol.dispose] = StaticNodegroup$1.prototype.free;
  ...
  let StaticResource$1 = class StaticResource2 {
    static __wrap(ptr) {
      ptr = ptr >>> 0;
      const obj = Object.create(StaticResource2.prototype);
      obj.__wbg_ptr = ptr;
      StaticResourceFinalization.register(obj, obj.__wbg_ptr, obj);
      return obj;
    }
    static __unwrap(jsValue) {
      if (!(jsValue instanceof StaticResource2)) {
        return 0;
      }
    ...
    static fromSummary(summary) {
      ...
      return StaticResource2.__wrap(ret);
    }
    set metadata(value) {
    ...
    static fromJsonString(json_str) {
      ...
      return StaticResource2.__wrap(ret[0]);
    }
    ...
    setTileDataForNode(tile_id, node_id, value) {
      const ptr0 = passStringToWasm0(tile_id, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
      const len0 = WASM_VECTOR_LEN;
      const ptr1 = passStringToWasm0(node_id, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
      const len1 = WASM_VECTOR_LEN;
      const ret = wasm.staticresource_setTileDataForNode(this.__wbg_ptr, ptr0, len0, ptr1, len1, value);
      return ret !== 0;
    }
  ...
  if (Symbol.dispose) StaticResource$1.prototype[Symbol.dispose] = StaticResource$1.prototype.free;
  ...
  class StaticResourceRegistry {
    ...
    memoryStats() {
      const ret = wasm.staticresourceregistry_memoryStats(this.__wbg_ptr);
      if (ret[2]) {
        throw takeFromExternrefTable0(ret[1]);
      }
      return takeFromExternrefTable0(ret[0]);
    ...
    memoryStatsDetailed() {
      const ret = wasm.staticresourceregistry_memoryStatsDetailed(this.__wbg_ptr);
      if (ret[2]) {
        throw takeFromExternrefTable0(ret[1]);
      }
      return takeFromExternrefTable0(ret[0]);
    ...
    loadFromBusinessDataBytes(bytes, store_full, include_caches) {
      const ptr0 = passArray8ToWasm0(bytes, wasm.__wbindgen_malloc);
      const len0 = WASM_VECTOR_LEN;
      const ret = wasm.staticresourceregistry_loadFromBusinessDataBytes(this.__wbg_ptr, ptr0, len0, isLikeNone(store_full) ? 16777215 : store_full ? 1 : 0, isLikeNone(include_caches) ? 16777215 : include_caches ? 1 : 0);
      if (ret[2]) {
        throw takeFromExternrefTable0(ret[1]);
      }
      return takeFromExternrefTable0(ret[0]);
    ...
    getFull(resource_id) {
      ...
      return ret === 0 ? void 0 : StaticResource$1.__wrap(ret);
    }
    hasFull(resource_id) {
      var ptr0 = isLikeNone(resource_id) ? 0 : passStringToWasm0(resource_id, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
      var len0 = WASM_VECTOR_LEN;
  ...
  let StaticTile$1 = class StaticTile2 {
    static __wrap(ptr) {
      ptr = ptr >>> 0;
      const obj = Object.create(StaticTile2.prototype);
      obj.__wbg_ptr = ptr;
      StaticTileFinalization.register(obj, obj.__wbg_ptr, obj);
      return obj;
    }
    static __unwrap(jsValue) {
      if (!(jsValue instanceof StaticTile2)) {
        return 0;
      }
      return jsValue.__destroy_into_raw();
    }
  ...
  if (Symbol.dispose) StaticTile$1.prototype[Symbol.dispose] = StaticTile$1.prototype.free;
  ...
  class WASMResourceInstanceWrapper {
    ...
    resolvePath(path) {
      ...
      const len0 = WASM_VECTOR_LEN;
      const ret = wasm.wasmresourceinstancewrapper_resolvePath(this.__wbg_ptr, ptr0, len0);
      if (ret[2]) {
        throw takeFromExternrefTable0(ret[1]);
      }
      return takeFromExternrefTable0(ret[0]);
    ...
    exportTilesJson() {
      ...
      let deferred2_1;
      try {
        const ret = wasm.wasmresourceinstancewrapper_exportTilesJson(this.__wbg_ptr);
        var ptr1 = ret[0];
        var len1 = ret[1];
        if (ret[3]) {
          ptr1 = 0;
    ...
    getValuesAtPath(path, filter_tile_id) {
      const ptr0 = passStringToWasm0(path, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
      const len0 = WASM_VECTOR_LEN;
      var ptr1 = isLikeNone(filter_tile_id) ? 0 : passStringToWasm0(filter_tile_id, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
      var len1 = WASM_VECTOR_LEN;
      const ret = wasm.wasmresourceinstancewrapper_getValuesAtPath(this.__wbg_ptr, ptr0, len0, ptr1, len1);
      if (ret[2]) {
        throw takeFromExternrefTable0(ret[1]);
      }
    ...
    setTileDataForNode(tile_id, node_id, value) {
      ...
      const ptr1 = passStringToWasm0(node_id, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
      const len1 = WASM_VECTOR_LEN;
      const ret = wasm.wasmresourceinstancewrapper_setTileDataForNode(this.__wbg_ptr, ptr0, len0, ptr1, len1, value);
      return ret !== 0;
    }
    ...
    getTile(tile_id) {
      ...
      return StaticTile$1.__wrap(ret[0]);
    }
  ...
  class WASMResourceModelWrapper {
    static __wrap(ptr) {
      ptr = ptr >>> 0;
    ...
    getRootNode() {
      ...
      return StaticNode$1.__wrap(ret[0]);
    }
    get edges() {
    ...
    isNodegroupPermitted(nodegroup_id, _tile) {
      ...
      let ptr1 = 0;
      if (!isLikeNone(_tile)) {
        _assertClass(_tile, StaticTile$1);
    ...
    get graph() {
      ...
      return StaticGraph$1.__wrap(ret);
    }
    set graph(graph) {
      _assertClass(graph, StaticGraph$1);
  ...
  class WKRM {
    ...
    get meta() {
      ...
      return StaticGraphMeta$1.__wrap(ret);
    }
    set meta(value) {
  ...
  const WasmCoercionResultFinalization = typeof FinalizationRegistry === "undefined" ? {
    register: () => {
    },
    unregister: () => {
    }
  } : new FinalizationRegistry((ptr) => wasm.__wbg_wasmcoercionresult_free(ptr >>> 0, 1));
  class WasmCoercionResult {
    static __wrap(ptr) {
      ptr = ptr >>> 0;
      const obj = Object.create(WasmCoercionResult.prototype);
      obj.__wbg_ptr = ptr;
      WasmCoercionResultFinalization.register(obj, obj.__wbg_ptr, obj);
      return obj;
    }
    __destroy_into_raw() {
      const ptr = this.__wbg_ptr;
      this.__wbg_ptr = 0;
      WasmCoercionResultFinalization.unregister(this);
      return ptr;
    }
    free() {
      const ptr = this.__destroy_into_raw();
      wasm.__wbg_wasmcoercionresult_free(ptr, 0);
    }
    get displayValue() {
      const ret = wasm.wasmcoercionresult_displayValue(this.__wbg_ptr);
      return ret;
    }
    get error() {
      const ret = wasm.wasmcoercionresult_error(this.__wbg_ptr);
      let v1;
      if (ret[0] !== 0) {
        v1 = getStringFromWasm0(ret[0], ret[1]).slice();
        wasm.__wbindgen_free(ret[0], ret[1] * 1, 1);
    ...
    get isNull() {
      const ret = wasm.wasmcoercionresult_isNull(this.__wbg_ptr);
      return ret !== 0;
    }
    get isError() {
      const ret = wasm.wasmcoercionresult_isError(this.__wbg_ptr);
      return ret !== 0;
    }
    get warnings() {
      const ret = wasm.wasmcoercionresult_warnings(this.__wbg_ptr);
      var v1 = getArrayJsValueFromWasm0(ret[0], ret[1]).slice();
      wasm.__wbindgen_free(ret[0], ret[1] * 4, 4);
      return v1;
    }
    get tileData() {
      const ret = wasm.wasmcoercionresult_tileData(this.__wbg_ptr);
      return ret;
    }
  }
  if (Symbol.dispose) WasmCoercionResult.prototype[Symbol.dispose] = WasmCoercionResult.prototype.free;
  ...
  const WasmNodegroupResultFinalization = typeof FinalizationRegistry === "undefined" ? {
    register: () => {
    },
    unregister: () => {
    }
  } : new FinalizationRegistry((ptr) => wasm.__wbg_wasmnodegroupresult_free(ptr >>> 0, 1));
  class WasmNodegroupResult {
    __destroy_into_raw() {
      const ptr = this.__wbg_ptr;
      this.__wbg_ptr = 0;
      WasmNodegroupResultFinalization.unregister(this);
      return ptr;
    }
    free() {
      const ptr = this.__destroy_into_raw();
      wasm.__wbg_wasmnodegroupresult_free(ptr, 0);
    }
    get valueCount() {
      const ret = wasm.wasmnodegroupresult_valueCount(this.__wbg_ptr);
      return ret >>> 0;
    }
    get impliedNodegroups() {
      const ret = wasm.wasmnodegroupresult_impliedNodegroups(this.__wbg_ptr);
      var v1 = getArrayJsValueFromWasm0(ret[0], ret[1]).slice();
      wasm.__wbindgen_free(ret[0], ret[1] * 4, 4);
      return v1;
    }
    getValue(index2) {
      const ret = wasm.wasmnodegroupresult_getValue(this.__wbg_ptr, index2);
      return ret === 0 ? void 0 : PseudoValue$1.__wrap(ret);
    }
  }
  if (Symbol.dispose) WasmNodegroupResult.prototype[Symbol.dispose] = WasmNodegroupResult.prototype.free;
  ...
  class WasmRdmCache {
    ...
    addCollectionsFromSkosXml(xml_content, base_uri) {
      const ptr0 = passStringToWasm0(xml_content, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
      const len0 = WASM_VECTOR_LEN;
      const ptr1 = passStringToWasm0(base_uri, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
      const len1 = WASM_VECTOR_LEN;
      const ret = wasm.wasmrdmcache_addCollectionsFromSkosXml(this.__wbg_ptr, ptr0, len0, ptr1, len1);
      if (ret[3]) {
        throw takeFromExternrefTable0(ret[2]);
      }
      var v3 = getArrayJsValueFromWasm0(ret[0], ret[1]).slice();
  ...
  class WasmSpan {
    ...
    setAttribute(key, value) {
      ...
      const len0 = WASM_VECTOR_LEN;
      wasm.wasmspan_setAttribute(this.__wbg_ptr, ptr0, len0, value);
    }
    end() {
      wasm.wasmspan_end(this.__wbg_ptr);
    }
    constructor(name) {
      const ptr0 = passStringToWasm0(name, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
      const len0 = WASM_VECTOR_LEN;
      const ret = wasm.wasmspan_new(ptr0, len0);
      this.__wbg_ptr = ret >>> 0;
      WasmSpanFinalization.register(this, this.__wbg_ptr, this);
      return this;
    }
  }
  ...
  function __wbg_get_imports() {
    ...
    imports.wbg.__wbg_clearMarks_67d59e45dcf56459 = function(arg0, arg1) {
    ...
    imports.wbg.__wbg_debug_d9d5d2b1cffc3bd5 = function(arg0, arg1, arg2) {
    ...
    imports.wbg.__wbg_forEach_859dfd887a0f866c = function(arg0, arg1, arg2) {
      try {
        ...
        var cb0 = (arg02, arg12) => {
          ...
          state0.a = 0;
          try {
            return __wbg_adapter_806(a, state0.b, arg02, arg12);
          } finally {
            state0.a = a;
          }
        };
    ...
    imports.wbg.__wbg_mark_0b30d85e14b55285 = function(arg0, arg1) {
      performance.mark(getStringFromWasm0(arg0, arg1));
    };
    imports.wbg.__wbg_measure_0a4fee7bc70b8938 = function(arg0, arg1, arg2) {
    ...
    imports.wbg.__wbg_now_05250764db9ed4a0 = function() {
    ...
    imports.wbg.__wbg_statictile_new = function(arg0) {
      const ret = StaticTile$1.__wrap(arg0);
      return ret;
    };
    imports.wbg.__wbg_statictile_unwrap = function(arg0) {
      const ret = StaticTile$1.__unwrap(arg0);
      return ret;
    };
  ...
  const wasmPkg = Object.freeze(Object.defineProperty({
    __proto__: null,
    ExampleEdgeWrapper,
    ExampleNodegroupWrapper,
    PseudoList: PseudoList$1,
    PseudoNode,
    PseudoValue: PseudoValue$1,
    StaticCard: StaticCard$1,
    StaticCardsXNodesXWidgets: StaticCardsXNodesXWidgets$1,
    StaticConstraint,
    StaticEdge: StaticEdge$1,
    StaticFunctionsXGraphs,
    StaticGraph: StaticGraph$1,
    StaticGraphMeta: StaticGraphMeta$1,
    StaticNode: StaticNode$1,
    StaticNodegroup: StaticNodegroup$1,
    StaticPublication,
    StaticResource: StaticResource$1,
    StaticResourceDescriptors,
    StaticResourceMetadata,
    StaticResourceReference,
    StaticResourceRegistry,
    StaticResourceSummary,
    StaticTile: StaticTile$1,
    StaticTranslatableString,
    WASMResourceInstanceWrapper,
    WASMResourceModelWrapper,
    WKRM,
    WasmCoercionResult,
    WasmEnsureNodegroupResult,
    ...
    WasmValuesFromNodegroupResult,
    addCollectionToGlobalRdmCache,
    addToGlobalRdmCacheFromSkosXml,
    applyMutationsCreate,
    applyMutationsFromJson,
    applyMutationsWithExtensions,
    batchMergeResources,
    batchTilesToTrees,
    batchTreesToTiles,
    buildAliasToCollectionMap,
    buildGraphFromModelCsvs: buildGraphFromModelCsvs$1,
    buildPrebuildExport,
    buildResourcesFromBusinessCsv: buildResourcesFromBusinessCsv$1,
    cardsToTree,
    clearGlobalRdmCache,
    clearRdmNamespace,
    clearRscvTimings,
    ...
    getRegisteredExtensionMutations,
    getRegisteredGraphIds,
    getRscvTimings,
    globalRdmGetCollectionIds,
    globalRdmGetParentId,
    globalRdmHasCollection,
    globalRdmLookupLabel,
    hasExtensionHandler,
    hasExtensionMutation,
    hasGlobalRdmCache,
    hasRdmNamespace,
    initSync,
    isValidUuid,
    labelsToDeterministicString,
    main,
    mergeResources,
    newWASMResourceInstanceWrapperForModel,
    ...
    registerExtensionMutation,
    removeFromGlobalRdmCache,
    ...
    unregisterExtensionMutation,
    validateModelCsvs: validateModelCsvs$1
  }, Symbol.toStringTag, {
    value: "Module"
  }));
  function _proxyClass(WasmClass, factoryFn) {
  ...
  const StaticGraph = _proxyClass(StaticGraph$1, createStaticGraph$1);
  const StaticGraphMeta = _proxyClass(StaticGraphMeta$1, (d) => createWasmType("StaticGraphMeta", d));
  const StaticNode = _proxyClass(StaticNode$1, createStaticNode);
  const StaticEdge = _proxyClass(StaticEdge$1, (d) => createWasmType("StaticEdge", d));
  const StaticTile = _proxyClass(StaticTile$1, (d) => createWasmType("StaticTile", d));
  const StaticResource = _proxyClass(StaticResource$1, (d) => createWasmType("StaticResource", d));
  const StaticNodegroup = _proxyClass(StaticNodegroup$1, (d) => createWasmType("StaticNodegroup", d));
  const StaticCard = _proxyClass(StaticCard$1, (d) => createWasmType("StaticCard", d));
  const StaticCardsXNodesXWidgets = _proxyClass(StaticCardsXNodesXWidgets$1, (d) => createWasmType("StaticCardsXNodesXWidgets", d));
  function createStaticGraph(props, published = true) {
    ...
    const descriptionValue = props.description ? typeof props.description === "object" && "toJSON" in props.description ? props.description : createStaticTranslatableString(props.description) : createStaticTranslatableString("");
    const rootCopy = typeof root.copy === "function" ? root.copy() : {
      ...root
    };
    return createStaticGraph$1({
      author: props.author || "",
      cards: null,
  ...
  class StaticValue {
    ...
    static create(referent, valueType, value, language) {
      const lang = language || getCurrentLanguage$2();
  ...
  class StaticConcept {
    ...
    getPrefLabel() {
      return this.prefLabels[getCurrentLanguage$2()] || Object.values(this.prefLabels)[0];
    }
    toString() {
    ...
    static fromValue(conceptScheme, value, children, config = {}) {
      let lang = (config == null ? void 0 : config.baseLanguage) || getCurrentLanguage$2();
      let tmpValue;
      let prefLabels = null;
  ...
  class StaticCollection {
    ...
    toString() {
      return (this.prefLabels[getCurrentLanguage$2()] || Object.values(this.prefLabels)[0] || "").toString();
    }
  }
  class StaticDomainValue {
    ...
    toString() {
      const lang = getCurrentLanguage$2();
  ...
  class ArchesClientRemoteStatic extends ArchesClient {
    ...
    async getCollection(collectionId) {
      ...
      const { parseSkosXmlToCollection: parseSkosXmlToCollection2, getRdmNamespaceRaw: getRdmNamespaceRaw2 } = await Promise.resolve().then(() => wasmPkg);
      const baseUri = getRdmNamespaceRaw2() || xmlResponse.url;
      return parseSkosXmlToCollection2(xmlText, baseUri);
    }
    async getResources(graphId, limit, _reloadIfSeen) {
      const resources = [];
      ...
      for await (const file of files) {
        ...
        const response = await fetch(source);
        const jsonText = await response.text();
        const resourceSet = parseStaticResources(jsonText);
  ...
  class ArchesClientLocal extends ArchesClient {
    ...
    async getResource(resourceId) {
      ...
      const resource = JSON.parse(response).business_data.resources.filter((resource2) => resource2.resourceinstance.resourceinstanceid === resourceId).map((resource2) => createStaticResource(resource2))[0];
      resource.__source = source;
      return resource;
    }
    async getCollection(collectionId) {
      ...
      try {
        ...
        if (getBackend() === "napi") {
          ...
          if (napi && typeof napi.parseSkosXmlToCollection === "function") {
            const xmlPath2 = jsonPath.replace(/\.json$/, ".xml");
            const xmlContent2 = await fs.promises.readFile(xmlPath2, "utf8");
            const baseUri2 = napi.getRdmNamespaceRaw && napi.getRdmNamespaceRaw() || "http://localhost/";
            return napi.parseSkosXmlToCollection(xmlContent2, baseUri2);
          }
        ...
        const { parseSkosXmlToCollection: parseSkosXmlToCollection2, getRdmNamespaceRaw: getRdmNamespaceRaw2 } = await Promise.resolve().then(() => wasmPkg);
        const baseUri = getRdmNamespaceRaw2() || "http://localhost/";
        return parseSkosXmlToCollection2(xmlContent, baseUri);
      }
    }
  ...
  function getLabelResolvableDatatypes() {
    if (_labelResolvableDatatypes === null) {
      _labelResolvableDatatypes = new Set(getDefaultResolvableDatatypes$1());
    }
  ...
  function getCollectionConfigKeys() {
    if (_collectionConfigKeys === null) {
      _collectionConfigKeys = getDefaultConfigKeys$1();
    }
  ...
  class ReferenceDataManager {
    ...
    async resolveLabels(tree, graph, options = {}) {
      ...
      const graphJson = safeStringify({
        graph: [
          graph
        ]
      });
      const aliasToCollection = buildAliasToCollectionMap$1(graphJson, resolvableDatatypes, configKeys);
      if (Object.keys(aliasToCollection).length === 0) {
        return tree;
      }
      const neededCollectionIds = findNeededCollections$1(treeJson, aliasToCollection);
      ...
      return cache.resolveLabels(treeJson, aliasToCollection, strict);
    }
  }
  ...
  CUSTOM_DATATYPES = /* @__PURE__ */ new Map();
  ...
  async function initWasm() {
    var _a2, _b2, _c2;
    if (!wasmInitialized) {
      if (getBackend() === "napi" || typeof process !== "undefined" && ((_b2 = (_a2 = process.env) == null ? void 0 : _a2.ALIZARIN_BACKEND) == null ? void 0 : _b2.toLowerCase()) === "napi") {
...
${possiblePaths.map((p) => `  - ${p}`).join("\n")}`);
        ...
        console.debug("[alizarin] Initializing WASM in browser environment", {
          wasmURL
        });
        try {
          await __wbg_init({
  ...
  _parseSkosXml = function(xmlContent, baseUri) {
    if (getBackend() === "napi") {
      const napi = getNapiModule();
      if (napi == null ? void 0 : napi.parseSkosXml) return napi.parseSkosXml(xmlContent, baseUri || "http://localhost/");
      throw new Error("parseSkosXml not available in NAPI binary");
    }
    return parseSkosXml(xmlContent, baseUri || "http://localhost/");
  };
  _parseSkosXmlToCollection = function(xmlContent, baseUri) {
    if (getBackend() === "napi") {
      const napi = getNapiModule();
      if (napi == null ? void 0 : napi.parseSkosXmlToCollection) return napi.parseSkosXmlToCollection(xmlContent, baseUri || "http://localhost/");
  ...
  _collectionToSkosXml = function(collection, baseUri) {
    const uri = baseUri || "http://localhost/";
    if (getBackend() === "napi") {
      const napi = getNapiModule();
      if (napi == null ? void 0 : napi.collectionToSkosXml) return napi.collectionToSkosXml(collection, uri);
      throw new Error("collectionToSkosXml not available in NAPI binary");
    }
    return collectionToSkosXml(collection, uri);
  };
  _collectionsToSkosXml = function(collections, baseUri) {
    const uri = baseUri || "http://localhost/";
    if (getBackend() === "napi") {
      const napi = getNapiModule();
      if (napi == null ? void 0 : napi.collectionsToSkosXml) return napi.collectionsToSkosXml(collections, uri);
  ...
  class NodeConfigManager {
    constructor() {
      __publicField(this, "_backendManager", null);
    ...
    getBackendManager() {
      if (!this._backendManager) {
        if (getBackend() === "napi") {
          const napi = globalThis.__alizarin_napi;
          if (napi) {
    ...
    loadFromGraph(graph) {
      const graphId = graph.graphid ?? graph.graphId;
      if (this._graphsLoaded.has(graphId)) return;
      const mgr = this.getBackendManager();
      const napi = getNapiModule();
      const isNapiGraph = (napi == null ? void 0 : napi.NapiStaticGraph) && graph instanceof napi.NapiStaticGraph;
    ...
    retrieve(node) {
      ...
      const mgr = this.getBackendManager();
      const config = typeof mgr.hasConfig === "function" && mgr.hasConfig(nodeid) ? this.getConfigFromWasm(nodeid, datatype) : null;
  ...
  const _GeoJSONViewModel = class _GeoJSONViewModel {
    ...
    toString() {
      ...
      const type = (val == null ? void 0 : val.type) || "GeoJSON";
      if (type === "FeatureCollection" && Array.isArray(val.features)) {
        const geomTypes = new Set(val.features.map((f2) => {
          var _a3;
          return (_a3 = f2 == null ? void 0 : f2.geometry) == null ? void 0 : _a3.type;
        }).filter(Boolean));
        if (geomTypes.size > 0) {
          return `${type} (${[
            ...geomTypes
      ...
      } else if (type === "Feature" && ((_a2 = val.geometry) == null ? void 0 : _a2.type)) {
  ...
  function checkCoordinateRanges(coords, warnings) {
  ...
  function validateGeoJSONCoordinates(value) {
    ...
    if (geoType === "FeatureCollection" && Array.isArray(value.features)) {
      for (const f2 of value.features) {
        warnings.push(...validateGeoJSONCoordinates(f2));
      }
  ...
  class NumberViewModel extends Number {
    ...
    static __create(tile, node, value) {
      ...
      console.debug(`[NumberViewModel.__create] nodeid=${nodeid} alias=${node.alias} value=`, value, `type=${typeof value} tile.tileid=${tile == null ? void 0 : tile.tileid} tile.data keys:`, (tile == null ? void 0 : tile.data) ? [
        ...tile.data.keys()
      ] : "no data");
      if (tile) {
        if (value !== null) {
  ...
  class SemanticViewModel {
    ...
    async toObject() {
      ...
      console.debug(`[semantic.toObject] node="${this.__node.alias || this.__node.nodeid}" parent_tile_id="${((_a2 = this.__tile) == null ? void 0 : _a2.tileid) || null}" parent_nodegroup_id="${this.__node.nodegroup_id || null}"`);
      const childValuesMap = wasmWrapper.getAllSemanticChildValues(((_b2 = this.__tile) == null ? void 0 : _b2.tileid) || null, this.__node.nodeid, this.__node.nodegroup_id || null);
      console.debug(`[semantic.toObject] node="${this.__node.alias || this.__node.nodeid}" childValuesMap keys:`, [
        ...childValuesMap.keys()
      ], "values:", [
        ...childValuesMap.entries()
      ].map(([k, v]) => {
        var _a3, _b3, _c2;
        return `${k}: ${(_a3 = v == null ? void 0 : v.constructor) == null ? void 0 : _a3.name} ${"getAllValues" in (v || {}) ? "len=" + ((_c2 = (_b3 = v.getAllValues) == null ? void 0 : _b3.call(v)) == null ? void 0 : _c2.length) : ""}`;
      }));
      timingStats.wasmCalls++;
    ...
    async __getChildValue(key) {
      ...
      const t0 = performance.now();
      try {
        console.debug(`[semantic.__getChildValue] key="${key}" parent_tile_id="${(tile == null ? void 0 : tile.tileid) || null}" parent_node_id="${node.nodeid}" parent_nodegroup_id="${node.nodegroup_id || null}" totalTiles=${(_a2 = wasmWrapper.getTileCount) == null ? void 0 : _a2.call(wasmWrapper)} nodegroupCount=${(_b2 = wasmWrapper.getNodegroupCount) == null ? void 0 : _b2.call(wasmWrapper)}`);
        rustValue = wasmWrapper.getSemanticChildValue((tile == null ? void 0 : tile.tileid) || null, node.nodeid, node.nodegroup_id || null, key);
        console.debug(`[semantic.__getChildValue] key="${key}" result:`, rustValue, "type:", (_c2 = rustValue == null ? void 0 : rustValue.constructor) == null ? void 0 : _c2.name, "getAllValues" in (rustValue || {}) ? `list length: ${(_e2 = (_d2 = rustValue.getAllValues) == null ? void 0 : _d2.call(rustValue)) == null ? void 0 : _e2.length}` : "not a list");
        timingStats.wasmCalls++;
        timingStats.wasmTotalMs += performance.now() - t0;
      } catch (e) {
  ...
  class PseudoValue {
    ...
    _updateValueReal() {
      ...
      if (currentTile && currentTile.data !== null && currentTile.data.has(nodeid) && snapshot.datatype !== "semantic") {
        ...
        console.debug(`[_updateValueReal] READ FROM SNAPSHOT alias=${snapshot.alias} nodeid=${nodeid} datatype=${snapshot.datatype} tileData=`, snapshot.tileData, `data=`, data, `hasTile=${!!currentTile} tileDataKeys=`, (currentTile == null ? void 0 : currentTile.data) ? [
          ...currentTile.data.keys()
        ] : "N/A");
      }
  ...
  GraphMutator = class {
    ...
    addCard(nodegroup, name, component, options = {}, config) {
      ...
      const helptext = (options == null ? void 0 : options.helptext) && (typeof options.helptext === "object" && options.helptext !== null && "toJSON" in options.helptext ? options.helptext : createStaticTranslatableString(options.helptext));
      const helptitle = (options == null ? void 0 : options.helptitle) && (typeof options.helptitle === "object" && options.helptitle !== null && "toJSON" in options.helptitle ? options.helptitle : createStaticTranslatableString(options.helptitle));
      const instructions = (options == null ? void 0 : options.instructions) && (typeof options.instructions === "object" && options.instructions !== null && "toJSON" in options.instructions ? options.instructions : createStaticTranslatableString(options.instructions));
  ...
  ResourceModelWrapper = class {
    constructor(wkrm, graph, viewModelClass, defaultAllow = false) {
      ...
      __publicField(this, "_nodegroups", null);
      this.wkrm = wkrm;
    ...
    getChildNodeAliases(nodeId) {
      return this._backend.getChildNodeAliases(nodeId);
    }
    getGraphId() {
      var _a2, _b2;
      return ((_b2 = (_a2 = this._backend).getGraphId) == null ? void 0 : _b2.call(_a2)) ?? this.wkrm.graphId;
    }
    getNodeObjectFromAlias(alias) {
    ...
    getNodegroupName(nodegroupId) {
      return this._backend.getNodegroupName(nodegroupId);
    }
    createPseudoNode(alias) {
    ...
    createPseudoNodeChild(childNode, parent) {
    ...
    setDefaultAllowAllNodegroups(defaultAllow) {
      this.pruneTiles = !defaultAllow;
    ...
    setPermittedNodegroups(permissions) {
      ...
      const permsForBackend = getBackend() === "napi" ? Object.fromEntries(this.permittedNodegroups) : this.permittedNodegroups;
  ...
  GraphManager = class {
    ...
    async loadGraph(modelClass, defaultAllow = true) {
      let modelClassName;
  ...
  buildGraphFromModelCsvs = function(graphCsv, nodesCsv, rdmNamespace, collectionsCsv) {
    if (getBackend() === "napi") {
      const napi = getNapiModule();
      if (napi == null ? void 0 : napi.buildGraphFromCsvs) {
        return napi.buildGraphFromCsvs(graphCsv, nodesCsv, collectionsCsv ?? null, rdmNamespace);
      }
    }
    return buildGraphFromModelCsvs$1(graphCsv, nodesCsv, collectionsCsv ?? null, rdmNamespace);
  };
  ...
  class Renderer extends BaseRenderer {
    ...
    async renderGeoJSON(value, _depth) {
  ...
  class MarkdownRenderer extends Renderer {
    constructor(callbacks) {
      ...
      __publicField(this, "geojsonToUrl");
    ...
    async renderGeoJSON(geojson, _depth) {
...
${value.split("\n").map((x) => `    ${x}`).join("\n")}
  ...
  _wrappedInitWasm = async function() {
    await _origInitWasm();
    _wasmReadyResolve();
  };
  Object.defineProperty(_wrappedInitWasm, "name", {
    value: "initWasm"
  });
  Promise.resolve().then(() => _wrappedInitWasm().catch((e) => {
    console.debug("[alizarin] Auto-init WASM deferred:", (e == null ? void 0 : e.message) || e);
  }));
...
export {
  AlizarinModel as A,
  getTimingStats as B,
  CUSTOM_DATATYPES as C,
  logTimingStats as D,
  index as E,
  _parseSkosXml as F,
  GraphManager as G,
  _parseSkosXmlToCollection as H,
  _collectionToSkosXml as I,
  _collectionsToSkosXml as J,
  CollectionMutator as K,
  ...
  viewModels as e,
  staticStore as f,
  getCurrentLanguage$2 as g,
  unregisterResolvableDatatype as h,
  renderers as i,
  interfaces as j,
  createWKRM as k,
  getWKRMClass as l,
  nodeConfig as m,
  nodeConfigManager as n,
  ResourceModelWrapper as o,
  GraphMutator as p,
  setCurrentLanguage as q,
  registerExtensionHandler$1 as r,
  staticTypes as s,
  getCurrentLanguage as t,
  utils as u,
  version as v,
  wasmReady as w,
  setWasmURL as x,
  ensureWasmRdmCache as y,
  resetTimingStats as z
};
