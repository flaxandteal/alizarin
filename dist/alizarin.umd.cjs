(function (global, factory) {
    ...
    (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.Alizarin = {}));
    ...
    function v5(value, namespace, buf, offset) {
        return v35(0x50, sha1, value, namespace);
    }
    v5.DNS = DNS;
    v5.URL = URL;
    ...
    class StaticConcept {
      ...
      static fromValue(conceptScheme, value, children, config = {}) {
        let lang = config?.baseLanguage || getCurrentLanguage$1();
        let tmpValue;
        let prefLabels;
    ...
    class NodeConfigManager {
      ...
      constructor(cache = void 0) {
        if (!cache) {
          cache = NodeConfigManager._cache || /* @__PURE__ */ new Map();
        }
        this.cache = cache;
      }
      retrieve(node) {
      ...
      static async __create(tile, node, value, cacheEntry) {
        ...
        if (tile) {
          ...
          if (value !== null) {
            ...
            if (!value) {
              val = null;
            } else if (value instanceof StaticValue) ; else if (value instanceof Promise) {
              return value.then((value2) => {
    ...
    class PseudoValue {
      ...
      updateValue(tile) {
        ...
        if (!this.tile) {
          ...
          if (this.inner) {
            return new AttrPromise(async (resolve) => {
              const tile2 = await this.inner?.getTile();
    ...
    class GraphMutator {
      ...
      addCard(nodegroup, name, component, options = {}, config) {
        ...
        const helptext = options?.helptext && (options.helptext instanceof StaticTranslatableString ? options.helptext : new StaticTranslatableString(options.helptext));
        const helptitle = options?.helptitle && (options.helptitle instanceof StaticTranslatableString ? options.helptitle : new StaticTranslatableString(options.helptitle));
        const instructions = options?.instructions && (options.instructions instanceof StaticTranslatableString ? options.instructions : new StaticTranslatableString(options.instructions));
      ...
      addWidgetToCard(nodeId, widget, name, config, options = {}) {
        this.mutations.push((graph) => {
          ...
          const card = graph.cards?.find((card2) => card2.nodegroup_id === node.nodegroup_id);
          if (card) {
...
${value.split("\n").map((x) => `    ${x}`).join("\n")}
    ...
    exports.AlizarinModel = AlizarinModel;
    exports.GraphManager = GraphManager;
    exports.GraphMutator = GraphMutator;
    exports.RDM = RDM;
    exports.ResourceModelWrapper = ResourceModelWrapper;
    exports.WKRM = WKRM;
    ...
    exports.staticTypes = staticTypes;
    exports.utils = utils;
