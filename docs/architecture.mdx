...
- **`ArchesClientRemote`**: Connects to live Arches servers via REST API (partial support)
...
```typescript
// 1. Initialize client (client classes live under the `client` namespace export)
