...
class ArchesClientRemote extends ArchesClient {
  ...
  async getResources(graphId: string, limit: number): Promise<StaticResource[]> {
    const response = await fetch(
...
class ArchesClientRemoteStatic extends ArchesClient {
  ...
  constructor(
    ...
    this.graphIdToGraphFile = graphIdToGraphFile || ((graphId: string) => `resource_models/${graphId}.json`);
    this.graphIdToResourcesFiles = graphIdToResourcesFiles || ((graphId: string) => [`business_data/_${graphId}.json`]);
    this.resourceIdToFile = resourceIdToFile || ((resourceId: string) => `business_data/${resourceId}.json`);
    this.collectionIdToFile = collectionIdToFile || ((collectionId: string) => `collections/${collectionId}.json`);
  }

  async getGraphs(): Promise<GraphResult> {
    const response = await fetch(
        `${this.archesUrl}/${this.allGraphFile()}`
    )
    return await response.json();
  }
  ...
  async getResources(graphId: string, limit: number): Promise<StaticResource[]> {
    let resources = [];
    for (let file of this.graphIdToResourcesFiles(graphId)) {
      const response = await fetch(
        `${this.archesUrl}/${file}`
      )
...
class ArchesClientLocal extends ArchesClient {
  ...
  constructor({
    ...
    this.graphIdToGraphFile = graphIdToGraphFile || ((graphId: string) => `public/models/${graphId}.json`);
    this.graphIdToResourcesFiles = graphIdToResourcesFiles || ((graphId: string) => [`public/resources/_${graphId}.json`]);
    this.resourceIdToFile = resourceIdToFile || ((resourceId: string) => `public/resources/${resourceId}.json`);
    this.collectionIdToFile = collectionIdToFile || ((collectionId: string) => `public/collections/${collectionId}.json`);
  }

  async getGraphs(): Promise<GraphResult> {
    const fs = await this.fs;
    const response = await fs.readFile(
        this.allGraphFile(), "utf8"
    )
  ...
  async getGraph(graphId: string): Promise<StaticGraph> {
    const fs = await this.fs;
    const response = await fs.readFile(
        this.graphIdToGraphFile(graphId), "utf8"
    )
  ...
  async getResource(resourceId: string): Promise<StaticResource> {
    const fs = await this.fs;
    const response = await fs.readFile(
      this.resourceIdToFile(resourceId), "utf8"
    )
    return await JSON.parse(response);
  }
  ...
  async getCollection(collectionId: string): Promise<StaticCollection> {
    const fs = await this.fs;
    const response = await fs.readFile(
      this.collectionIdToFile(collectionId), "utf8"
    )
    return await JSON.parse(response);
  }


  async getResources(graphId: string, limit: number | null): Promise<StaticResource[]> {
    const fs = await this.fs;
    let resources = [];
    for (let file of this.graphIdToResourcesFiles(graphId)) {
      const response = await fs.readFile(
...
export { archesClient, ArchesClient, ArchesClientRemoteStatic, ArchesClientRemote, ArchesClientLocal };
