...
class SemanticViewModel implements IStringKeyedObject, IViewModel {
  ...
  async __getChildValue(key: string, setDefault: boolean = false): Promise<IPseudo> {
    ...
    if (!this.__childValues.has(key)) {
      ...
      child.parentValue = this.__parentPseudo || null;
    } else {
      ...
      if (childNode) {
        for (let value of values) {
          if (
            ...
            value.node &&
            (!(value.parentValue) ||
              value.parentValue === this.__parentPseudo)
          ) {
            ...
            if (
              // value.nodegroup_id == node.nodeid in all cases for first possibility?
              ...
              // (value.nodegroup_id == node.nodegroup_id &&
              //   tile &&
              //   value.tile == tile &&
              //   !childNode.is_collector) //  # It shares a tile
            ) {
