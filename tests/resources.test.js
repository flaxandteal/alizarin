...
describe('testing api', () => {
  ...
  apiTest('loads resource models', async ({ graphsResponse, graphResponses }) => {
    ...
    Object.values(graphResponses).forEach((response) => fetch.once(JSON.stringify(response)));
  ...
  apiTest('loads resource models lazily', async ({ graphsResponse, graphResponses }) => {
    fetch.once(JSON.stringify(graphsResponse))
    // Order doesn't strictly matter as long as it gets all, but it would be nicer to map to the request.
    Object.values(graphResponses).forEach((response) => fetch.once(JSON.stringify(response)));
