/// Tests for RustPseudoList::matching_entries
///
...
use alizarin::pseudo_value::{RustPseudoList, RustPseudoValue};
...
/// Create a RustPseudoValue with the specified tile
fn create_pseudo_value(node: Arc<StaticNode>, tile: Arc<StaticTile>) -> RustPseudoValue {
    RustPseudoValue::from_node_and_tile(
        node,
        Some(tile),
...
fn test_matching_entries_handles_child_tiles() {
    ...
    // Create outer tile
    let outer_tile = create_tile("tile-outer", outer_nodegroup, None);
